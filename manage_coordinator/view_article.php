...
if (!empty($_SESSION)) {
    ...
    $studentSubmission = $conn->query("SELECT user.username, faculty.f_name from `user` INNER JOIN faculty on user.faculty_id = faculty.f_id where `u_id` = '$idStudent'");
    $student = mysqli_fetch_assoc($studentSubmission);
    $studentName = $student['username'];
    $studentFaculty = $student['f_name'];
    ...
    function zipFilesAndDownload($file_names, $studentName, $facultyName)
    {
    ...
    $userSubmission = $conn->query("SELECT file_submit_to_submission.*, user.*,faculty.* FROM file_submit_to_submission INNER JOIN user ON file_submit_to_submission.file_userId_uploaded = user.u_id INNER JOIN faculty ON faculty.f_id = user.faculty_id WHERE user.role = 'student' AND user.faculty_id = '$userFacultyId'  ORDER BY id DESC LIMIT 1");

    $mailSend = mysqli_fetch_assoc($userSubmission);
...
?>
                        ...
                        <div class="col-m-8 content-form">
                            ...
                            if ($fileContent->num_rows > 0) {
                                foreach ($fileContentArray as $row) {
                                    ...
                                    $downfile[] = '../student/file_library/' . $row['file_content_name'];
                                    ...
                                    if ($fileType === "docx") {
                                        ...
                                        // $fileType = "pdf";
                                        // $fileURL =  str_replace('docx', $fileType, $fileURL);
                                        ...
                                        $fileURLs = '/student/file_library/' . $row["file_content_name"];
                                        $url = 'https://ciliweb.vn/coursework' . $fileURLs;


                            ?>
                                        <br /><br />
                                        <!-- <iframe src="https://docs.google.com/viewer?url<?= $url ?>s&embedded=true"></iframe> -->
                                        <iframe src="https://docs.google.com/viewer?url=<?= $url ?>&embedded=true" style="width:600px; height:500px;" frameborder="0"></iframe>
                                    <?php
                                    ...
                                    ?>
                                        ...
                                        <div id="my_pdf_viewer">
                                            <div id="canvas_container" style="width: 800px;height: 600px; overflow: auto;">
                                ...
                                if (isset($_POST['downFile'])) {
                                    ...
                                    zipFilesAndDownload($downfile,  $studentName, $studentFaculty);
                                }
                                ?>
                        ...
                        <div class="col-m-4 feedback-form">
                            ...
                            <div class="infor-student-submission">
                                ...
                                if ($fileSubmission->num_rows > 0) {
                                    while ($rowFile =  mysqli_fetch_array($fileSubmission)) {
                                ?>
                                        <div>
    ...
    if (isset($_POST['uploadCommnet'])) {
        ...
        if ($uploadCmt == true) {
            ...
            $message = "Please check your feedback on the contribution application website";
            $subject = "You have received feedback from the manager-coordinator";
            $text_message    =  "feedback has sending";
